...
ui <- dashboardPage(
    dashboardHeader(title = "Word checker"),
    ...
    dashboardBody(
        tabItems(
            tabItem(tabName = "Most_occuring_words",
                    fluidRow(box(uiOutput("name")), box(uiOutput("Days"))),
  ...
  output$Country <- renderUI({
    ...
    output$Days <- renderUI({
        ...
        selectInput(inputId = "Days",
                    label = "Days",
                    choices = days_selector_df)
    })
    output$name <- renderUI({
      
      name_df <- ase %>% filter(Country == input$Country & Days ==input$Days ) %>% 
            select(name) %>%  
            distinct() %>% 
            arrange(name)
      
        # newspaper_selector_df <- ase %>% 
        #     filter(Country == input$Country & Days ==input$Days ) %>% 
        #     select(name) %>% 
        #     distinct() %>% 
        #     arrange(name)
...
shared_data <- ase %>% group_by(word) %>% 
  mutate(occurrences=sum(Number)) %>% 
  arrange(desc(occurrences)) %>% 
  top_n(10) %>% ungroup() %>% SharedData$new()
...
bscols(widths = c(3, 9),
       list(
            filter_checkbox(id = "Days", label = "Days",
                    sharedData = shared_data, group = ~Days),
            filter_select(id = "Country", label = "Country",
                    sharedData = shared_data, group = ~Country)),d)
              ...
              ,column(# width should be between 1 and 12
               ...
               box(dataTableOutput("table1"),
                   title="Most Common Words Across used in Newspapers across countries",
                    ...
                    # This overrides the default value
                    width=NULL)
               ),box(uiOutput("name")), box(uiOutput("Days"))),
                    box(uiOutput("Country"))
                    )
        )
    )
...
server <- function(input, output) {  
 ...
 #   
   ...
   output$plot1 <- plotly::renderPlotly({
     plotly::ggplotly(
     ase %>% filter(Country == input$Country & Days ==input$Days & name ==input$name) %>% group_by(word) %>% 
       summarise(occurrences=sum(Number)) %>% 
     arrange(desc(occurrences)) %>% 
      top_n(input$select_topn) %>%
      ungroup() %>% 
   ...
   output$table1 <- renderDataTable({
     ase %>% filter(Country == input$Country & Days ==input$Days & name ==input$name) %>%
     group_by(word) %>% 
   summarise(occurrences=sum(Number)) %>% 
    arrange(desc(occurrences)) %>% 
    top_n(input$select_topn) %>%
