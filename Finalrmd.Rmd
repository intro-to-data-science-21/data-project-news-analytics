...
author: 
- Abhiram Mohan ([Github](https://github.com/Abhilearns2code)) | 205928
- Koen Oosthoek ([Github](https://github.com/koenoosthoek))      | 204638
output: 
  html_document:
    toc: FALSE
    df_print: paged
...
Headlines in newspapers are extremely important because a good headline attracts readers and can lead them to read the content written below.This makes it necessary to use words in a fashion which makes the headline appealing to the viewer.We attempt to find out how long these words stay in contention during a certain period starting from a time when their usages in headlines were the highest.For this purpose we take a look at newspapers from five different predominantly English speaking countries such as UK,USA,New Zealand,Australia and Canada.Across the different countries we conducted our research it is observed that Covid-19 and its new variant omicron remains a hot topic and stays on the headlines through out the duration of our research.Yet many words that were dominating the headlines at the start of our scraping loses its significance in the days that followed.
...
It is often said that "breaking news", speculative and sensational headlines characterize the modern 24-hour news cycle. In order to survive today's fast-paced media landscape, outlets need to compete for audiences that subscribe to their platform. Some authors underline the dangers this may pose to trustworthy information and an an informed public debate more generally - and refer to the "menace of media speed and the 24-hour news cycle". ^[Rosenberg, H., & Feldman, C. S. (2008). No Time To Think. Bloomsbury Academic.] If outlets want to survive in the market, they need to compete and attract as many users as possible. The gain for outlets is twofold: if more users subscribe to a platform, this does not just raise revenue in itself, but also makes the platform more attractive to advertisers. 

**Methodology : **

We want to deepen our understanding of the modern news-cycle. In this article, we will research how media headlines develop over time. We chose to focus on the words that are used in the front page headlines of the largest newspapers in the US, UK, Canada, Australia and New-Zealand. We scraped over a million front page headlines between December 9 00:01 CET and December 16 20:00 CET, with a time-interval of four hours. This allows us to observe how often words change between time intervals or days. Through this analysis, we aim to gain more insight the question whether the headlines of the news-cycle are constantly changing - adapting to the latest developments - or like to stay in the news-cycle for a longer period of time.
...
Us_New <- c("The Washington Post","The Los Angeles Times","The Wallstreet Journal","The Newyork Times","The Tampa bay times","USA Today")
...
us <-us%>% 
  kable(col.names = c("Newspapers","Headlines")) %>% 
...
UK_New <-c("The Daily Mail","The Mirror","The Telegraph","The Independent","The Times","The Guardian")
...
Canada_new <-c("The Globe and Mail","The Toronto Star","The Vancouver Sun","The Montreal Gazette","The Toronto Sun","The National Post")
...
canada <- canada %>% kable(col.names = c("Newspapers","Headlines")) %>% 
  kable_styling(bootstrap_options = c("striped", "hover"),
                full_width = TRUE,
                position = "center")
```
...
aus_new <-c("The West Australian","The Courier Mail","The Daily Telegraph","The Advertiser","The Herald Sun","The Financial Review")
...
aus <- aus %>% kable(col.names = c("Newspapers","Headlines")) %>% 
  kable_styling(bootstrap_options = c("striped", "hover"),
                full_width = TRUE,
                position = "center")
```
...
nz_new <-c("The New Zealand Herald","Courier Mail","Daily Telegraph","Advertiser","Herald Sun","Financial Review")
...
##
Turning to *figure 2*, we can observe the twenty words that were most used in headlines. Our results show one consistent and self-explanatory finding: "Covid" and "Christmas" are the most prominent words in the headlines between December 9 and 15. From December 11 onwards, the word "omicron" joins the top-3 and moves to number 2 on December 14. Other health-related words such as "vaccine" and "booster" (from December 13) are also highly prevalent. Moreover, words relating to justice and law enforcement are often mentioned. "Police", "court", "dead", and "killed" are used relatively frequently on all days. Other topics such as inflation, 
...
graph_us <- graph_us_data %>% 
  filter(word == "biden" | word == "omicron" | word == "covid" | word == "court" | word == "holiday" |
         ...
         word == "health" | word == "politce" | word == "putin" | word == "school" | word == "booster" | word == "democrats") %>% 
  ggplot(aes(x = time, y = n, group = word, colour = word)) +
  geom_line() +
  geom_point(size = 0.4) +
...
graph_uk <- graph_uk_data %>% 
    filter(word == "christmas" | word == "covid" | word == "party"  | word == "johnson" |
           ...
           word == "fans" | word == "death" | word == "government" | word == "husband" | word == "baby" |
           word == "vaccine" | word == "police" | word == "festive" | word == "tory") %>% 
  ggplot(aes(x = time, y = n, group = word, colour = word)) +
  geom_line() +
  geom_point(size = 0.4) +
...
graph_canada <- graph_canada_data %>% 
  filter(word == "covid" | word == "canadian" | word == "million" | word == "car" | word == "ontario" | word == "boycott" | word == "beijing" |
           word == "diplomatic" | word == "omicron" | word == "drug" | word == "fight" | word == "hit" |
...
graph_australia <- graph_australia_data %>% 
  filter(word == "covid" | word == "ashes" | word == "aussie" | word == "fight" |
           ...
           word == "death" | word == "hits" | word == "suimmer") %>% 
  ggplot(aes(x = time, y = n, group = word, colour = word)) +
  geom_line() +
  geom_point(size = 0.4) +
...
graph_nz <- graph_nz_data %>% 
  filter(word == "christmas" | word == "covid" | word == "auckland" | word == "gifts" | word == "business" | word == "wellington" |
           ...
           word == "black" | word == "gift" | word == "guide" | word == "plans" |
           word == "bejing" | word == "boss" | word == "canterbury" | word == "coach" | word == "company" |
           word == "december" | word == "kiwi" | word == "light") %>% 
  ggplot(aes(x = time, y = n, group = word, colour = word)) +
  geom_line() +
  geom_point(size = 0.4) +
...
Until now, we have visualized and discussed the frequency of certain words between December 9 and 15. However, in order to understand how long certain words remain in the news cycle over time, we focus on a range of words - more specifically, the words that were discussed most often at our starting time point, December 9 at 0.01. This is what *figure 3* shows: the "frequency development" of the words that were most present in the headlines on December 9. Looking first at the aggregated data of all countries, we can see that Covid is consistently prevalent in headlines. In our time period, the use of the word Christmas decreases substantially from December 13 onwards. On December 13 at 12.00 CET, the use of the word omikron increases substantially. However, for most words (e.g. "plan", "car", "police"), we observe the following trend: while they are discussed relatively often on the first time point, their use decreases constantly until our last time point. 

However, on a country level, we observe that the UK is the only country that follows the aggregated trend - most likely due to the fact that the headlines of UK newspapers make up a large part of the dataset. In all other four countries, we observe interesting diverging trends. In the US, 6 out of 13 words (booster, death, defense, democrats, health and Putin) lose salience after December 9. In Canada, the use of 13 out of 21 words (Alzheimer's, Beijing, boycott, car, diplmatic, drug, fight, fire, health, hit, investors, money and olympics) decrease substantially after December 9. In Australia, just 2 out of 16 words dissapear from the headlines after December 9: the other 14 words are present relatively constant in the news headlines. In New-Zealand, 13 out of 19 words lose significance in the course of time. 

**Conclusion : **

On the basis of these findings, we can conclude that in the researched time period, the speed or variability of the news-cycle is the highest in the UK, Canada and New-Zealand. In these countries, the topics in the headlines that we identified lost significance relatively quickly - especially in comparison to the US and Australia. In these countries, most words remained  salient over the time course. Apart from these findings, it is important to reiterate that our short time span of seven days is prevents us from seeing the bigger picture. Our data is insufficient to draw  strong conclusions - however, if anything, we can safely say that December 2021 will be remembered for Chrstimas and Covid.

**Contributions : **



We both were equally involved in brainstorming how we want to present the data we scrape in a convincing manner. This involved deciding on which countries to focus on and the visualisations to choose when we present the data. We had initially divided the work into two where Koen will do the scraping part and Abhiram will work on the visualisations.<br/>

Koen created the function which scraped and cleaned the headlines. He created the line graphs in figure 2 and the table 1 as well.<br/>

 Abhiram created a shiny dashboard initially which would allow users to filter between countries, days and newspaper type to find out the words most used on a particular day. On further discussions we decided to leave out the dashboard and present the data in a simpler format.(Please find a working dashboard (shiny_dashboard.rmd) in the project directory).Abhiram created the figure 1.<br/>

Both were involved in the creation of the final document.